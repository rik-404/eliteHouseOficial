...
export default function Schedule() {
  ...
  // Filtrar eventos - admin e dev veem todos, corretores veem apenas os seus
  const events = useMemo(() => {
    if (!user) return [];
    // Se for admin ou dev, retorna todos os eventos
    if (user.role === 'admin' || user.role === 'dev') {
      return allEvents;
    }
    // Caso contrário, filtra apenas os eventos do corretor logado
    return allEvents.filter(event => event.broker_id === user.id);
  }, [allEvents, user]);
